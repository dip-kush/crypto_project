...
int check_max(BigInteger A, BigInteger B){
	...
	for(i=N-1; i>=0; i--) {
		if(A.digits[i]>B.digits[i]){      // if the most signifant digit is greater
			return 1;
		}
...
BigInteger subtract( BigInteger A, BigInteger B){ 
	...
	}else if(A.sign == POSITIVE && B.sign == NEGATIVE){   // eg subtract(548, -458)
		B.sign = POSITIVE;
...
BigInteger add (BigInteger A, BigInteger B) {
	...
	C.length = N - i;
			...
			else if(!boo && A.sign == NEGATIVE){     // |A| < |B| and A is negative
				A.sign = POSITIVE;
				C = subtract(B, A);		
			}
			else if(boo && A.sign == POSITIVE){   	 // |A| > |B| and A is positive
				B.sign = POSITIVE;
				C = subtract(A, B);
			}else{                                   // |A|  < |B|  and A is negative
				A.sign = POSITIVE; 
				C = subtract(A, B);
				C.sign = NEGATIVE;
			}
...
int mul_inv(int a, int b)
{
        ...
        if (b < 0) b = -b;
        if (a < 0) a = b - (-a % b);
 		t = 0;  nt = 1;  r = b;  nr = a % b;																																																																																																																																																																																																																																																																																																																																																																		       t = 0;  nt = 1;  r = b;  nr = a % b;
        while (nr != 0) {
          q = r/nr;
